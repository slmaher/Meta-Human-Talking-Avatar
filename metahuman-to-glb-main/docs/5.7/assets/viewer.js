...
export async function mount(container, opts) {
  ...
  try {
    ...
    console.warn('[viewer] WebGLRenderer default failed, attempting fallback without antialias...', e);
    try {
      renderer = new THREE.WebGLRenderer({
        antialias: false,
        alpha: false,
      ...
      console.error('[viewer] Fatal WebGL context creation error:', e2);
      throw new Error('Error creating WebGL context. The browser GPU process has reached its active context limit. Please refresh or close background WebGL tabs.');
    }
  }
